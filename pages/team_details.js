...
const TeamDetails = (props)=>{
    ...
    function handleEditSubmit(e)
    {
        ...
        let feed = {
            ...editData,
            'Flag Image': roster[edit.index]['Flag Image'],
            'Appearances': roster[edit.index]['Appearances'],
            'Starter':roster[edit.index]['Starter'],
            'Minutes Played': roster[edit.index]['Minutes Played'],
        }
        if(!feed['Player Name'])
            feed['Player Name'] = roster[edit.index]['Player Name'];
        if(!feed['Jersey Number'])
            feed['Jersey Number'] = roster[edit.index]['Jersey Number'];
        if(!feed['Height'])
            feed['Height'] = roster[edit.index]['Height']
        if(!feed['Weight'])
            feed['Weight'] = roster[edit.index]['Weight']
        if(!feed['Nationality'])
            feed['Nationality'] = roster[edit.index]['Nationality'];
        if(!feed['Position'])
            feed['Position'] = roster[edit.index]['Position'];
    ...
    {
        let deleteUpdate = roster;
        deleteUpdate.splice(Delete.index,1);
        setRoster(deleteUpdate);
        ...
        <>
            <main className="details-container">
                ...
                <div className="player-details">
                        <TeamTable Roster={roster} Edit={setEdit} Delete={setDelete}/>
                                        ...
                                        <div className="edit-modal-body">
                                            <div className="field-one">
                                                <div className="field-name">
                                                    <p>Player Name</p>
                                                    <input onChange={handleEditChange} id="edit-player-name" type="text" defaultValue={roster[edit.index]['Player Name']} value={editData['Player Name']}/>
                                                </div>
                                                <div className="jersey-field">
                                                    <p>Jersey Number</p>
                                                    <input onChange={handleEditChange} id="edit-player-number" type="text" defaultValue={roster[edit.index]['Jersey Number']} value={editData['Jersey Number']}/>
                                                </div>
                                            </div>
                                            <div className="field-two">
                                                <div>
                                                    <p>Height</p>
                                                    <input onChange={handleEditChange} id="edit-height" type="text" defaultValue={roster[edit.index]['Height']} value={editData.Height}/>
                                                </div>
                                                <div>
                                                    <p>Weight</p>
                                                    <input onChange={handleEditChange} id="edit-weight" type="text" defaultValue={roster[edit.index]['Weight']} value={editData.Weight} />
                                                </div>
                                            </div>
                                                ...
                                                <select id="edit-nation" onChange={handleEditChange}>
                                                   {
                                                        countries.map((item,index)=>{
                                                            return(
                                                                item === roster[edit.index]['Nationality'] ? <option key={index} value={item} selected>{item}</option> : <option key={index} value={item}>{item}</option>
                                                            )
                                                        })
                                                   }
                                                </select>
                                                ...
                                                <select id="edit-position" onChange={handleEditChange}>
                                                    {
                                                        positions.map((item,index)=>{
                                                            return(
                                                                item === roster[edit.index]['Position'] ? <option key={index} value={item} selected>{item}</option> : <option key={index} value={item}>{item}</option>
                                                            )
                                                        })
                                                   }
                                                </select>
